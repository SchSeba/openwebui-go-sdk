/*
Open WebUI

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 0.1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the QueryCollectionsForm type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &QueryCollectionsForm{}

// QueryCollectionsForm struct for QueryCollectionsForm
type QueryCollectionsForm struct {
	CollectionNames []string `json:"collection_names"`
	Query string `json:"query"`
	K NullableInt32 `json:"k,omitempty"`
	KReranker NullableInt32 `json:"k_reranker,omitempty"`
	R NullableFloat32 `json:"r,omitempty"`
	Hybrid NullableBool `json:"hybrid,omitempty"`
	HybridBm25Weight NullableFloat32 `json:"hybrid_bm25_weight,omitempty"`
}

type _QueryCollectionsForm QueryCollectionsForm

// NewQueryCollectionsForm instantiates a new QueryCollectionsForm object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewQueryCollectionsForm(collectionNames []string, query string) *QueryCollectionsForm {
	this := QueryCollectionsForm{}
	this.CollectionNames = collectionNames
	this.Query = query
	return &this
}

// NewQueryCollectionsFormWithDefaults instantiates a new QueryCollectionsForm object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewQueryCollectionsFormWithDefaults() *QueryCollectionsForm {
	this := QueryCollectionsForm{}
	return &this
}

// GetCollectionNames returns the CollectionNames field value
func (o *QueryCollectionsForm) GetCollectionNames() []string {
	if o == nil {
		var ret []string
		return ret
	}

	return o.CollectionNames
}

// GetCollectionNamesOk returns a tuple with the CollectionNames field value
// and a boolean to check if the value has been set.
func (o *QueryCollectionsForm) GetCollectionNamesOk() ([]string, bool) {
	if o == nil {
		return nil, false
	}
	return o.CollectionNames, true
}

// SetCollectionNames sets field value
func (o *QueryCollectionsForm) SetCollectionNames(v []string) {
	o.CollectionNames = v
}

// GetQuery returns the Query field value
func (o *QueryCollectionsForm) GetQuery() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Query
}

// GetQueryOk returns a tuple with the Query field value
// and a boolean to check if the value has been set.
func (o *QueryCollectionsForm) GetQueryOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Query, true
}

// SetQuery sets field value
func (o *QueryCollectionsForm) SetQuery(v string) {
	o.Query = v
}

// GetK returns the K field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *QueryCollectionsForm) GetK() int32 {
	if o == nil || IsNil(o.K.Get()) {
		var ret int32
		return ret
	}
	return *o.K.Get()
}

// GetKOk returns a tuple with the K field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *QueryCollectionsForm) GetKOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return o.K.Get(), o.K.IsSet()
}

// HasK returns a boolean if a field has been set.
func (o *QueryCollectionsForm) HasK() bool {
	if o != nil && o.K.IsSet() {
		return true
	}

	return false
}

// SetK gets a reference to the given NullableInt32 and assigns it to the K field.
func (o *QueryCollectionsForm) SetK(v int32) {
	o.K.Set(&v)
}
// SetKNil sets the value for K to be an explicit nil
func (o *QueryCollectionsForm) SetKNil() {
	o.K.Set(nil)
}

// UnsetK ensures that no value is present for K, not even an explicit nil
func (o *QueryCollectionsForm) UnsetK() {
	o.K.Unset()
}

// GetKReranker returns the KReranker field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *QueryCollectionsForm) GetKReranker() int32 {
	if o == nil || IsNil(o.KReranker.Get()) {
		var ret int32
		return ret
	}
	return *o.KReranker.Get()
}

// GetKRerankerOk returns a tuple with the KReranker field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *QueryCollectionsForm) GetKRerankerOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return o.KReranker.Get(), o.KReranker.IsSet()
}

// HasKReranker returns a boolean if a field has been set.
func (o *QueryCollectionsForm) HasKReranker() bool {
	if o != nil && o.KReranker.IsSet() {
		return true
	}

	return false
}

// SetKReranker gets a reference to the given NullableInt32 and assigns it to the KReranker field.
func (o *QueryCollectionsForm) SetKReranker(v int32) {
	o.KReranker.Set(&v)
}
// SetKRerankerNil sets the value for KReranker to be an explicit nil
func (o *QueryCollectionsForm) SetKRerankerNil() {
	o.KReranker.Set(nil)
}

// UnsetKReranker ensures that no value is present for KReranker, not even an explicit nil
func (o *QueryCollectionsForm) UnsetKReranker() {
	o.KReranker.Unset()
}

// GetR returns the R field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *QueryCollectionsForm) GetR() float32 {
	if o == nil || IsNil(o.R.Get()) {
		var ret float32
		return ret
	}
	return *o.R.Get()
}

// GetROk returns a tuple with the R field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *QueryCollectionsForm) GetROk() (*float32, bool) {
	if o == nil {
		return nil, false
	}
	return o.R.Get(), o.R.IsSet()
}

// HasR returns a boolean if a field has been set.
func (o *QueryCollectionsForm) HasR() bool {
	if o != nil && o.R.IsSet() {
		return true
	}

	return false
}

// SetR gets a reference to the given NullableFloat32 and assigns it to the R field.
func (o *QueryCollectionsForm) SetR(v float32) {
	o.R.Set(&v)
}
// SetRNil sets the value for R to be an explicit nil
func (o *QueryCollectionsForm) SetRNil() {
	o.R.Set(nil)
}

// UnsetR ensures that no value is present for R, not even an explicit nil
func (o *QueryCollectionsForm) UnsetR() {
	o.R.Unset()
}

// GetHybrid returns the Hybrid field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *QueryCollectionsForm) GetHybrid() bool {
	if o == nil || IsNil(o.Hybrid.Get()) {
		var ret bool
		return ret
	}
	return *o.Hybrid.Get()
}

// GetHybridOk returns a tuple with the Hybrid field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *QueryCollectionsForm) GetHybridOk() (*bool, bool) {
	if o == nil {
		return nil, false
	}
	return o.Hybrid.Get(), o.Hybrid.IsSet()
}

// HasHybrid returns a boolean if a field has been set.
func (o *QueryCollectionsForm) HasHybrid() bool {
	if o != nil && o.Hybrid.IsSet() {
		return true
	}

	return false
}

// SetHybrid gets a reference to the given NullableBool and assigns it to the Hybrid field.
func (o *QueryCollectionsForm) SetHybrid(v bool) {
	o.Hybrid.Set(&v)
}
// SetHybridNil sets the value for Hybrid to be an explicit nil
func (o *QueryCollectionsForm) SetHybridNil() {
	o.Hybrid.Set(nil)
}

// UnsetHybrid ensures that no value is present for Hybrid, not even an explicit nil
func (o *QueryCollectionsForm) UnsetHybrid() {
	o.Hybrid.Unset()
}

// GetHybridBm25Weight returns the HybridBm25Weight field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *QueryCollectionsForm) GetHybridBm25Weight() float32 {
	if o == nil || IsNil(o.HybridBm25Weight.Get()) {
		var ret float32
		return ret
	}
	return *o.HybridBm25Weight.Get()
}

// GetHybridBm25WeightOk returns a tuple with the HybridBm25Weight field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *QueryCollectionsForm) GetHybridBm25WeightOk() (*float32, bool) {
	if o == nil {
		return nil, false
	}
	return o.HybridBm25Weight.Get(), o.HybridBm25Weight.IsSet()
}

// HasHybridBm25Weight returns a boolean if a field has been set.
func (o *QueryCollectionsForm) HasHybridBm25Weight() bool {
	if o != nil && o.HybridBm25Weight.IsSet() {
		return true
	}

	return false
}

// SetHybridBm25Weight gets a reference to the given NullableFloat32 and assigns it to the HybridBm25Weight field.
func (o *QueryCollectionsForm) SetHybridBm25Weight(v float32) {
	o.HybridBm25Weight.Set(&v)
}
// SetHybridBm25WeightNil sets the value for HybridBm25Weight to be an explicit nil
func (o *QueryCollectionsForm) SetHybridBm25WeightNil() {
	o.HybridBm25Weight.Set(nil)
}

// UnsetHybridBm25Weight ensures that no value is present for HybridBm25Weight, not even an explicit nil
func (o *QueryCollectionsForm) UnsetHybridBm25Weight() {
	o.HybridBm25Weight.Unset()
}

func (o QueryCollectionsForm) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o QueryCollectionsForm) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["collection_names"] = o.CollectionNames
	toSerialize["query"] = o.Query
	if o.K.IsSet() {
		toSerialize["k"] = o.K.Get()
	}
	if o.KReranker.IsSet() {
		toSerialize["k_reranker"] = o.KReranker.Get()
	}
	if o.R.IsSet() {
		toSerialize["r"] = o.R.Get()
	}
	if o.Hybrid.IsSet() {
		toSerialize["hybrid"] = o.Hybrid.Get()
	}
	if o.HybridBm25Weight.IsSet() {
		toSerialize["hybrid_bm25_weight"] = o.HybridBm25Weight.Get()
	}
	return toSerialize, nil
}

func (o *QueryCollectionsForm) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"collection_names",
		"query",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varQueryCollectionsForm := _QueryCollectionsForm{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varQueryCollectionsForm)

	if err != nil {
		return err
	}

	*o = QueryCollectionsForm(varQueryCollectionsForm)

	return err
}

type NullableQueryCollectionsForm struct {
	value *QueryCollectionsForm
	isSet bool
}

func (v NullableQueryCollectionsForm) Get() *QueryCollectionsForm {
	return v.value
}

func (v *NullableQueryCollectionsForm) Set(val *QueryCollectionsForm) {
	v.value = val
	v.isSet = true
}

func (v NullableQueryCollectionsForm) IsSet() bool {
	return v.isSet
}

func (v *NullableQueryCollectionsForm) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableQueryCollectionsForm(val *QueryCollectionsForm) *NullableQueryCollectionsForm {
	return &NullableQueryCollectionsForm{value: val, isSet: true}
}

func (v NullableQueryCollectionsForm) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableQueryCollectionsForm) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


